...
import dash
from dash import Dash, html, dcc, Input, Output, callback, State, callback_context, ALL
...
def update_content(pathname, search, dropdown_value):
    ...
    # This block runs if the callback was triggered by a URL change
        ...
        if pathname == '/' or pathname == '/suites':
            ...
            sim = platform.get_item(sim_id, ItemType.SIMULATION)
            sim_dir = str(platform.get_directory(sim))
            if not row.empty:
                return html.Div([
                    dcc.Store(id='current-path', data={'path': sim_dir}),
...
def update_directory_view(path, directories, files):
    # Create a list item for going up one level
    up_button = html.Li(html.Button("Up", id={'type': 'dir-link', 'index': '..'}, n_clicks=0))

    # Create list items for each directory
    directory_links = [
        html.Li(html.A(os.path.basename(d), href='#', id={'type': 'dir-link', 'index': os.path.join(path, d)}))
        for d in directories
    ]

    # Create list items for each file
    file_items = [html.Li(os.path.basename(f)) for f in files]

    # Combine all items into one list under a single Ul element
    return html.Ul([up_button] + directory_links + file_items)

@callback(
    Output('file-list', 'children'),
    Input('current-path', 'data')
)
def update_file_list(current_data):
    ctx = dash.callback_context
    ...
    return update_directory_view(path, directories, files)
    # return html.Ul([
    #     #html.Li(html.Button("Up", id={'type': 'dir-link', 'index': '..'}, n_clicks=0)),
    #     html.Li(html.A(os.path.basename(d), href='#', id={'type': 'dir-link', 'index': os.path.join(path, d)}))
    #     for d in directories
    # ] + [html.Li(os.path.basename(f)) for f in files])
...
def update_current_path(clicks, current_data):
    ...
    try:
        clicked_dir_dict = json.loads(clicked_element)
        clicked_dir = clicked_dir_dict['index']
    except json.JSONDecodeError as e:
        print(f"Error decoding JSON: {e}")
        raise PreventUpdate  # Prevent the callback from updating if there's an error
    if clicked_dir == '..':
        new_path = os.path.dirname(current_data)
    else:
        new_path = os.path.abspath(clicked_dir)

    return render_directory_contents(new_path)
