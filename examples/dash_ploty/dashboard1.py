...
def generate_table_data(dataframe):
    ...
    dataframe['details_link'] = dataframe['sim_id'].apply(lambda x: f'[{x}](/details/{x})')
    ...
    return dataframe[['details_link', 'status', 'last_modified', 'plot_link']].to_dict('records')
...
def display_page(pathname):
    if pathname.startswith('/details/'):
        ...
        if not row.empty:
            ...
            ]), generate_table_data(row), {'height': '300px', 'overflowY': 'auto'}
        return html.Div("No data available for this Simulation ID."), [], {'height': '300px', 'overflowY': 'auto'}
    elif pathname.startswith('/plot/'):
        sim_id = pathname.split('/')[-1]
        row = df[df['sim_id'] == sim_id]
        return dcc.Graph(figure={}), generate_table_data(row), {'height': '100px', 'overflowY': 'auto'} # Placeholder for actual plot logic
    return "Select a Simulation ID to view details or an Inset Chart to plot.",generate_table_data(df), {'height': '300px', 'overflowY': 'auto'}
