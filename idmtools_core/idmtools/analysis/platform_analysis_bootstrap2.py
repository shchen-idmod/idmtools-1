...
if __name__ == "__main__":
    ...
    analyzer_data = pickle.load(open(r"analyzers.pkl", 'rb'))
    analyzers = []
    for data in analyzer_data:
        # Dynamically import the class using the module name and class name
        A = locate(f"{data['module_name']}.{data['class_name']}")

        if A is None:
            raise ValueError(f"Class {data['class_name']} not found in module {data['module_name']}")

        # Create a new instance using the configuration
        a = A(**data["config"])
        analyzers.append(a)
